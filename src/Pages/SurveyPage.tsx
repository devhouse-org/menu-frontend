  ...
  const handleRatingSelect = (questionId: string, score: number) => {
    setRatings((prevRatings) => {
      ...
      if (existingRatingIndex !== -1) {
        // If the rating for this questionId already exists, update it
        const updatedRatings = [...prevRatings];
        updatedRatings[existingRatingIndex].score = score;
          ...
          {data.map((q: any) => {
            return (
              <RatingSelector
                titleEn={q.enTitle}
                titleAr={q.title}
                onSelect={(rating) => handleRatingSelect(q.id, rating)}
